...
for process = processes
    ...
    for i = 1:length(sampleSizes)
        ...
        for s=1:numSims
            X = X_full(1:n, s);
            Y = Y_full(1:n, s);
            partialResults(s) = wildHSIC(X,Y,'Test',1,'NumBootstrap',1000).reject;
        end           
        toc
...
for process = processes
    ...
    for i = 1:length(sampleSizes)
        ...
        for s=1:numSims
            X = X_full(1:n, s);
            Y = Y_full(1:n, s);
            partialResults(s) = wildHSIC(X,Y,'Test',2,'NumBootstrap',1000).reject;
        end           
        toc
        powers(i, :) = [n, mean(partialResults)];
        disp(powers(1:i, :));
    end
    filename = sprintf("power_curves/wildHSIC2_powers_%s.mat", process);
    save(filename,'powers')
end
toc
%delete(pool)